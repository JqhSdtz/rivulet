...
public class HibernateMissingSequenceChangeGenerator extends MissingSequenceChangeGenerator {
    ...
    public Change[] fixMissing(DatabaseObject missingObject, DiffOutputControl control, Database referenceDatabase, Database comparisonDatabase, ChangeGeneratorChain chain) {
        if (referenceDatabase instanceof RvDatabase && !comparisonDatabase.supportsSequences()) {
            return null;
        } else if (comparisonDatabase instanceof RvDatabase && !referenceDatabase.supportsSequences()) {
            return null;
        }else {
