...
import {pickDataProps, usePrefixCls} from '@formily/antd/lib/__builtins__';
import {useFormLayout} from '@formily/antd/lib/form-layout';
import cls from 'classnames';
import {useSize} from 'ahooks';
import {TabNodeContext} from "@/layouts/BasicLayout";
import {useFormShallowLayout} from "@/components/formily";
...
export const FormGrid: ComposedFormGrid = observer(
    ({children, className, style, ...props}: React.PropsWithChildren<IFormGridProps>) => {
        const layout = useFormShallowLayout();
        const options = {
        ...
        const gridSize = useSize(ref);
        const splitViewSize = useSize(tabNode.splitView.contentRef);
        const layoutSize = useSize(layout.ref);
        if (gridSize && gridSize.width !== 0) {
            grid.width = gridSize.width;
            grid.height = gridSize.height;
        } else if (layoutSize && layoutSize.width !== 0) {
            grid.width = layoutSize.width;
        } else if (splitViewSize && splitViewSize.width !== 0) {
            grid.width = splitViewSize.width;
        }
        useLayoutEffect(() => {
